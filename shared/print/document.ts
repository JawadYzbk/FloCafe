...
export interface PaymentSnapshot {
  ...
  /**
   * Optional secondary-currency tender annotation. When a payment was taken in
   * a currency other than the store base (e.g. cash paid in LBP against a USD
   * bill), these carry the amount and code actually tendered so receipts can
   * show "Cash (22,250,000 LBP)". The `amount` column stays in base currency.
   */
  readonly tenderCurrency?: string;
...
export interface PaymentsBlock {
  ...
  readonly lines: readonly {
    ...
    readonly label: SemanticLabel;
    readonly amount: number;
    /** Secondary-currency tender annotation, if the line was paid in one. */
    readonly tenderCurrency?: string;
    readonly tenderAmount?: number;
  }[];
}
