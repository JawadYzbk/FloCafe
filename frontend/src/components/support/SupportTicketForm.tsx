...
export interface SupportTicketEndpoints {
  profile: string;
  submit: string;
  statusBase: string;
}

export const AUTHENTICATED_SUPPORT_ENDPOINTS: SupportTicketEndpoints = {
  profile: '/support-ticket/profile',
  submit: '/support-ticket',
  statusBase: '/support-ticket',
};

export const PRE_LOGIN_SUPPORT_ENDPOINTS: SupportTicketEndpoints = {
  profile: '/support-ticket/pre-login/profile',
  submit: '/support-ticket/pre-login',
  statusBase: '/support-ticket/pre-login',
};
...
interface SupportTicketFormProps {
  endpoints?: SupportTicketEndpoints;
...
export function SupportTicketForm({
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (attachLog && typeof window !== 'undefined' && window.electronAPI?.getLogTail) {
        const result = await window.electronAPI.getLogTail().catch(() => null);
        if (result && 'text' in result) logTail = result.text;
      }
      const { data } = await api.post(endpoints.submit, {
        category,
        severity,
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <label className="flex items-start gap-2 text-sm text-muted-foreground select-none cursor-pointer">
              ...
              />
              <span>
                <span className="block text-foreground">{t('attachLog')}</span>
                <span className="block text-xs">{t('attachLogHint')}</span>
              </span>
            </label>
